...
var require_set_interval_async_timer = __commonJS({
	"node_modules/set-interval-async/dist/set-interval-async-timer.cjs"(
		...
		var _timeout,
			...
			_scheduleTimeout,
			scheduleTimeout_fn,
			_runHandlerAndScheduleTimeout,
			runHandlerAndScheduleTimeout_fn;
		...
		runHandlerAndScheduleTimeout_fn = async function (
			strategy,
			handler,
			intervalMs,
			...handlerArgs
	...
	"node_modules/set-interval-async/dist/clear-interval-async.cjs"(exports) {
		...
		async function clearIntervalAsync3(timer) {
			if (
				!(
					timer instanceof
					set_interval_async_timer_cjs_1.SetIntervalAsyncTimer
				)
			) {
				throw new TypeError(
			...
			await set_interval_async_timer_cjs_1.SetIntervalAsyncTimer.stopTimer(
				timer
			);
		}
...
var require_set_interval_async = __commonJS({
	"node_modules/set-interval-async/dist/dynamic/set-interval-async.cjs"(
		...
		Object.defineProperty(exports, "clearIntervalAsync", {
			enumerable: true,
			get: function () {
		...
		function setIntervalAsync2(handler, intervalMs, ...handlerArgs) {
			...
			return set_interval_async_timer_cjs_1.SetIntervalAsyncTimer.startTimer(
				"dynamic",
				handler,
				intervalMs,
				...handlerArgs
		...
		for (let key of __getOwnPropNames2(module2))
			if (
				...
				__defProp2(target, key, {
					get: () => module2[key],
					enumerable:
						!(desc = __getOwnPropDesc2(module2, key)) ||
						desc.enumerable,
				});
	}
	return target;
...
var __toCommonJS2 = /* @__PURE__ */ ((cache2) => {
	return (module2, temp) => {
		return (
			(cache2 && cache2.get(module2)) ||
			((temp = __reExport(__markAsModule({}), module2, 1)),
			cache2 && cache2.set(module2, temp),
			temp)
		);
	};
...
function bufferToString(input) {
	return (Array.isArray(input) ? Buffer.concat(input) : input).toString(
		"utf-8"
	);
}
function pick(source, properties) {
	return Object.assign(
		...
		...properties.map((property) =>
			property in source ? { [property]: source[property] } : {}
		)
	);
}
...
var init_util = __esm({
	"src/lib/utils/util.ts"() {
		NULL = "\0";
		NOOP = () => {};
		objectToString = Object.prototype.toString.call.bind(
...
function filterPrimitives(input, omit) {
	...
	return (
		/number|string|boolean/.test(type) && (!omit || !omit.includes(type))
	);
}
...
var init_argument_filters = __esm({
	"src/lib/utils/argument-filters.ts"() {
		...
		filterStringOrStringArray = (input) => {
			return (
				filterString(input) ||
				(Array.isArray(input) && input.every(filterString))
			);
		};
		filterHasLength = (input) => {
			if (
				input == null ||
				"number|boolean|function".includes(typeof input)
			) {
				return false;
			}
			return (
...
var init_line_parser = __esm({
	"src/lib/utils/line-parser.ts"() {
		LineParser = class {
			constructor(regExp, useMatches) {
				...
				this.parse = (line, target) => {
					this.resetMatches();
					if (
						!this._regExp.every((reg, index) =>
							this.addMatch(reg, index, line(index))
						)
					) {
						return false;
					}
					return (
						this.useMatches(target, this.prepareMatches()) !== false
					);
				};
		...
		RemoteLineParser = class extends LineParser {
			addMatch(reg, index, line) {
				return (
					/^remote:\s/.test(String(line)) &&
					super.addMatch(reg, index, line)
				);
			}
...
function getTrailingOptions(args, initialPrimitive = 0, objectOnly = false) {
	const command = [];
	for (
		let i = 0, max = initialPrimitive < 0 ? args.length : initialPrimitive;
		i < max;
		i++
	) {
...
function trailingOptionsArgument(args) {
	...
	return filterType(
		last(args, hasTrailingCallback ? 1 : 0),
		filterPlainObject
	);
}
...
function parseStringResponse(result, parsers12, texts, trim = true) {
	asArray(texts).forEach((text) => {
		for (
			let lines = toLinesWithContent(text, trim),
				i = 0,
				max = lines.length;
			i < max;
			i++
...
function cleanSummaryParser(dryRun, text) {
	...
	toLinesWithContent(text).forEach((line) => {
		...
		(isFolderRegexp.test(removed) ? summary.folders : summary.files).push(
			removed
		);
	});
	return summary;
	...
	return {
		...
		parser() {
			throw typeof error === "string"
				? new TaskConfigurationError(error)
				: error;
		},
	};
}
...
function cleanWithOptionsTask(mode, customArgs) {
	...
	if (!valid.options) {
		return configurationErrorTask(
			CONFIG_ERROR_UNKNOWN_OPTION + JSON.stringify(mode)
		);
	}
...
function isCleanOptionsArray(input) {
	return (
		Array.isArray(input) &&
		input.every((test) => CleanOptionValues.has(test))
	);
}
	...
	input
		...
		.forEach((char) => {
			if (isCleanMode(char)) {
				...
				valid.options =
					valid.options &&
					isKnownOption((options[options.length] = `-${char}`));
			}
		});
	return {
		cleanMode,
		...
		ConfigList = class {
			...
			addFile(file) {
				if (!(file in this.values)) {
					...
					this.values[file] = latest
						? Object.create(this.values[latest])
						: {};
				...
				if (options.includes(option)) {
					return this._runTask(
						configurationErrorTask(
							`git.grep: use of "${option}" is not supported.`
						),
						then
					);
				}
...
function createLogger(label, verbose, initialStep, infoDebugger = createLog()) {
	...
	const key = childLoggerName(
		filterType(verbose, filterString),
		debugDebugger,
		infoDebugger
	);
	...
	function step(phase) {
		...
		const debug2 =
			(debugDebugger && prefixedLogger(debugDebugger, stepPrefix)) ||
			NOOP;
		const info = prefixedLogger(
			infoDebugger,
...
var init_tasks_pending_queue = __esm({
	"src/lib/runners/tasks-pending-queue.ts"() {
		...
		_TasksPendingQueue = class {
			...
			fatal(err) {
				for (const [task, { logger }] of Array.from(
					this._queue.entries()
				)) {
					if (task === err.task) {
		...
		GitExecutorChain = class {
			...
			attemptRemoteTask(task, logger) {
				return __async(this, null, function* () {
					...
					const outputStreams = yield this.handleTaskData(
						task,
						args,
						raw,
						logger.step("HANDLE")
					...
					logger(
						`passing response to task's parser as a %s`,
						task.format
					);
					if (isBufferTask(task)) {
						return callTaskParser(task.parser, outputStreams);
					}
					return callTaskParser(
						task.parser,
						outputStreams.asStrings()
					);
				});
			}
			...
			handleTaskData(task, args, result, logger) {
				...
				return new Promise((done, fail) => {
					...
					const { error } = this._plugins.exec(
						...
						{ error: rejection },
						__spreadValues(
							__spreadValues({}, pluginContext(task, args)),
							result
						)
					);
					if (error && task.onError) {
						logger.info(
							`exitCode=%s handling with custom error handler`
						);
						return task.onError(
							result,
							error,
							(newStdOut) => {
								logger.info(
									`custom error handler treated as success`
								);
								logger(
									`custom error returned a %s`,
									objectToString(newStdOut)
								);
								done(
									new GitOutputStreams(
										Array.isArray(newStdOut)
											? Buffer.concat(newStdOut)
											: newStdOut,
	...
	"src/lib/runners/git-executor.ts"() {
		...
		GitExecutor = class {
			constructor(binary = "git", cwd, _scheduler, _plugins) {
				...
				this._scheduler = _scheduler;
				this._plugins = _plugins;
				this._chain = new GitExecutorChain(
					this,
					this._scheduler,
					this._plugins
				);
...
function taskCallback(task, response, callback = NOOP) {
	...
	const onError2 = (err) => {
		if ((err == null ? void 0 : err.task) === task) {
			callback(
				err instanceof GitResponseError
					? addDeprecationNoticeToError(err)
					: err,
				void 0
			);
		}
...
function addDeprecationNoticeToError(err) {
	...
	return Object.create(
		err,
		Object.getOwnPropertyNames(err.git).reduce(descriptorReducer, {})
	);
...
function changeWorkingDirectoryTask(directory, root) {
	return adhocExecTask((instance) => {
		if (!folderExists(directory)) {
			throw new Error(
				`Git.cwd: cannot change to non-directory "${directory}"`
			);
		}
...
function getDiffParser(format = "") {
	...
	return (stdOut) =>
		parseStringResponse(new DiffSummary(), parser3, stdOut, false);
}
var statParser;
var numStatParser;
var nameOnlyParser;
		...
		statParser = [
			...
			new LineParser(
				...
				(result, [changed, summary]) => {
					...
					result.insertions = asNumber(
						inserted == null ? void 0 : inserted[1]
					);
					result.deletions = asNumber(
						deleted == null ? void 0 : deleted[1]
					);
				}
			),
		];
...
function logTask(splitter, fields, customArgs) {
	const parser3 = createListLogSummaryParser(
		splitter,
		fields,
		logFormatFromCommand(customArgs)
	);
	return {
...
var init_log = __esm({
	"src/lib/tasks/log.ts"() {
		...
		excludeOptions = /* @__PURE__ */ ((excludeOptions2) => {
			...
			excludeOptions2[(excludeOptions2["strictDate"] = 12)] =
				"strictDate";
		...
		remoteMessagesObjectParsers = [
			new RemoteLineParser(
				...
				(result, [action, count]) => {
					...
					const enumeration = objectEnumerationResult(
			...
			new RemoteLineParser(
				...
				(result, [total, reused, packReused]) => {
					const objects = objectEnumerationResult(
						result.remoteMessages
					);
...
function parsePullErrorResult(stdOut, stdErr) {
	const pullError = parseStringResponse(
		new PullFailedSummary(),
		errorParsers,
		[stdOut, stdErr]
	);
	...
	"src/lib/parsers/parse-pull.ts"() {
		...
		parsers3 = [
			...
			new LineParser(ACTION_REGEX, (result, [action, file]) => {
				...
				append(
					action === "create" ? result.created : result.deleted,
					file
				);
			}),
		];
		errorParsers = [
			new LineParser(
				/^from\s(.+)$/i,
				(result, [remote]) => void (result.remote = remote)
			),
			new LineParser(
				/^fatal:\s(.+)$/,
				(result, [message]) => void (result.message = message)
			),
			new LineParser(
				/([a-z0-9]+)\.\.([a-z0-9]+)\s+(\S+)\s+->\s+(\S+)$/,
				(
					result,
					[hashLocal, hashRemote, branchLocal, branchRemote]
				) => {
		...
		parsePullDetail = (stdOut, stdErr) => {
			return parseStringResponse(new PullSummary(), parsers3, [
				stdOut,
				stdErr,
			]);
		};
			...
			new LineParser(
				...
				(summary, [reason, file]) => {
					summary.conflicts.push(
						new MergeSummaryConflict(reason, file)
					);
				}
			),
			new LineParser(
				...
				(summary, [reason, file, deleteRef]) => {
					summary.conflicts.push(
						new MergeSummaryConflict(reason, file, { deleteRef })
					);
				}
			),
	...
	"src/lib/parsers/parse-push.ts"() {
		...
		parsers5 = [
			...
			new LineParser(
				...
				(result, [local]) => {
					result.ref = __spreadProps(
						__spreadValues({}, result.ref || {}),
						{
							local,
						}
					);
			...
			new LineParser(
				/^[=*-]\s+([^:]+):(\S+)\s+\[(.+)]$/,
				(result, [local, remote, type]) => {
					result.pushed.push(
						pushResultPushedItem(local, remote, type)
					);
				}
			),
			new LineParser(
				...
				(result, [local, remote, remoteName]) => {
					result.branch = __spreadProps(
						__spreadValues({}, result.branch || {}),
						{
							local,
							remote,
							remoteName,
		...
		parsePushResult = (stdOut, stdErr) => {
			...
			return __spreadValues(
				__spreadValues({}, pushDetail),
				responseDetail
			);
		};
		parsePushDetail = (stdOut, stdErr) => {
			return parseStringResponse({ pushed: [] }, parsers5, [
				stdOut,
				stdErr,
			]);
		};
...
function conflicts(indexX, ...indexY) {
	return indexY.map((y) =>
		parser2(indexX, y, (result, file) => append(result.conflicted, file))
	);
}
	...
	"src/lib/responses/StatusSummary.ts"() {
		...
		parsers6 = new Map([
			...
			parser2(
				...
				" ",
				(result, file) =>
					append(result.created, file) && append(result.staged, file)
			),
			parser2(
				"A",
				"M",
			...
			parser2(
				...
				" ",
				(result, file) =>
					append(result.deleted, file) && append(result.staged, file)
			),
			parser2(
				"M",
				" ",
				(result, file) =>
					append(result.modified, file) && append(result.staged, file)
			),
			parser2(
				"M",
				"M",
			...
			[
				...
				(result, line) => {
					...
					result.current =
						(regexResult && regexResult[1]) || result.current;
		...
		SimpleGitApi = class {
			...
			cwd(directory) {
				...
				if (typeof directory === "string") {
					return this._runTask(
						changeWorkingDirectoryTask(directory, this._executor),
						next
					);
				}
				if (
					typeof (directory == null ? void 0 : directory.path) ===
					"string"
				) {
					return this._runTask(
...
var init_scheduler = __esm({
	"src/lib/runners/scheduler.ts"() {
		...
		createScheduledTask = (() => {
			...
			return () => {
				id++;
				const { promise, done } = (0,
				import_promise_deferred.createDeferred)();
				return {
					promise,
					done,
					id,
...
var init_parse_branch_delete = __esm({
	"src/lib/parsers/parse-branch-delete.ts"() {
		...
		parseBranchDeletions = (stdOut, stdErr) => {
			return parseStringResponse(new BranchDeletionBatch(), parsers8, [
				stdOut,
				stdErr,
			]);
		};
...
function cloneTask(repo, directory, customArgs) {
	...
	if (banned) {
		return configurationErrorTask(
			`git.fetch: potential exploit argument blocked.`
		);
	}
	return straightThroughStringTask(commands);
}
		...
		parseTagList = function (data, customSort = false) {
			...
			if (!customSort) {
				tags.sort(function (tagA, tagB) {
					...
					for (
						let i = 0, l = Math.max(partsA.length, partsB.length);
						i < l;
						i++
					) {
						const diff = sorted(
...
var require_git = __commonJS2({
	"src/git.js"(exports, module2) {
		var { GitExecutor: GitExecutor2 } =
			(init_git_executor(), __toCommonJS2(git_executor_exports));
		var { SimpleGitApi: SimpleGitApi2 } =
			(init_simple_git_api(), __toCommonJS2(simple_git_api_exports));
		var { Scheduler: Scheduler2 } =
			(init_scheduler(), __toCommonJS2(scheduler_exports));
		var { configurationErrorTask: configurationErrorTask2 } =
		...
		var { applyPatchTask: applyPatchTask2 } =
		...
		var { checkIgnoreTask: checkIgnoreTask2 } =
			(init_check_ignore(), __toCommonJS2(check_ignore_exports));
		var { checkIsRepoTask: checkIsRepoTask2 } =
			(init_check_is_repo(), __toCommonJS2(check_is_repo_exports));
		var { cloneTask: cloneTask2, cloneMirrorTask: cloneMirrorTask2 } =
		...
		var { diffSummaryTask: diffSummaryTask2 } =
			(init_diff(), __toCommonJS2(diff_exports));
		var { fetchTask: fetchTask2 } =
			(init_fetch(), __toCommonJS2(fetch_exports));
		var { moveTask: moveTask2 } =
			(init_move(), __toCommonJS2(move_exports));
		var { pullTask: pullTask2 } =
			(init_pull(), __toCommonJS2(pull_exports));
		var { pushTagsTask: pushTagsTask2 } =
			(init_push(), __toCommonJS2(push_exports));
		var {
		...
		var { getResetMode: getResetMode2, resetTask: resetTask2 } =
			(init_reset(), __toCommonJS2(reset_exports));
		var { stashListTask: stashListTask2 } =
			(init_stash_list(), __toCommonJS2(stash_list_exports));
		var {
		...
		(Git2.prototype = Object.create(SimpleGitApi2.prototype)).constructor =
			Git2;
		...
		function createCloneTask(api, task, repoPath, localPath) {
			if (typeof repoPath !== "string") {
				return configurationErrorTask2(
					`git.${api}() requires a string 'repoPath'`
				);
			}
			return task(
				repoPath,
				filterType2(localPath, filterString2),
		...
		Git2.prototype.revert = function (commit) {
			...
			if (typeof commit !== "string") {
				return this._runTask(
					configurationErrorTask2("Commit must be a string"),
					next
				);
			}
			return this._runTask(
		...
		Git2.prototype.deleteLocalBranch = function (
			branchName,
			forceDelete,
			then
		) {
			return this._runTask(
				deleteBranchTask2(
					branchName,
					typeof forceDelete === "boolean" ? forceDelete : false
				),
				trailingFunctionArgument2(arguments)
			);
		};
		Git2.prototype.deleteLocalBranches = function (
			branchNames,
			forceDelete,
			then
		) {
		...
		Git2.prototype.raw = function (commands) {
			...
			const command = [].slice.call(
				createRestCommands ? arguments : commands,
				0
			);
			...
			return this._runTask(
				straightThroughStringTask2(command, this._trimmed),
				next
			);
		};
		Git2.prototype.submoduleAdd = function (repo, path, then) {
			return this._runTask(
				addSubModuleTask2(repo, path),
				trailingFunctionArgument2(arguments)
			);
		};
		...
		Git2.prototype.removeRemote = function (remoteName, then) {
			return this._runTask(
				removeRemoteTask2(remoteName),
				trailingFunctionArgument2(arguments)
			);
		};
		Git2.prototype.getRemotes = function (verbose, then) {
			return this._runTask(
				getRemotesTask2(verbose === true),
				trailingFunctionArgument2(arguments)
			);
		};
		...
		Git2.prototype.tag = function (options, then) {
			...
			return this._runTask(
				straightThroughStringTask2(command),
				trailingFunctionArgument2(arguments)
			);
		};
		...
		Git2.prototype._catFile = function (format, args) {
			...
			const task =
				format === "buffer"
					? straightThroughBufferTask2(command)
					: straightThroughStringTask2(command);
		...
		Git2.prototype.applyPatch = function (patches) {
			const task = !filterStringOrStringArray2(patches)
				...
				: applyPatchTask2(
						asArray2(patches),
						getTrailingOptions2([].slice.call(arguments, 1))
				  );
			return this._runTask(task, trailingFunctionArgument2(arguments));
		};
		Git2.prototype.revparse = function () {
			const commands = [
				"rev-parse",
				...getTrailingOptions2(arguments, true),
			];
			return this._runTask(
		...
		Git2.prototype.clean = function (mode, options, then) {
			...
			const cleanMode =
				(usingCleanOptionsArray && mode.join("")) ||
				filterType2(mode, filterString2) ||
				"";
			const customArgs = getTrailingOptions2(
		...
		Git2.prototype.checkIgnore = function (pathnames, then) {
			return this._runTask(
				checkIgnoreTask2(
					asArray2(
						filterType2(pathnames, filterStringOrStringArray2, [])
					)
				),
				trailingFunctionArgument2(arguments)
			);
...
function abortPlugin(signal) {
	...
	const onSpawnAfter = {
		...
		action(_data, context) {
			function kill() {
				context.kill(
					new GitPluginError(void 0, "abort", "Abort signal received")
				);
			}
			signal.addEventListener("abort", kill);
			context.spawned.on("close", () =>
		...
		action(args, context) {
			args.forEach((current, index) => {
				...
				allowUnsafeProtocolOverride ||
					preventProtocolOverride(current, next);
	...
	return {
		...
		action(_0, _1) {
			return __async(
				this,
				arguments,
				function* (_data, { spawned, close }) {
					var _a2, _b;
					const events = createEvents();
					let deferClose = true;
					let quickClose = () => void (deferClose = false);
					(_a2 = spawned.stdout) == null
						? void 0
						: _a2.on("data", quickClose);
					(_b = spawned.stderr) == null
						? void 0
						: _b.on("data", quickClose);
...
var PluginStore = class {
	...
	add(plugin) {
		...
		asArray(plugin).forEach(
			(plugin2) => plugin2 && this.plugins.add(append(plugins, plugin2))
		);
		return () => {
	...
	if (block > 0) {
		return {
			...
			action(_data, context) {
				...
				function stop() {
					var _a3, _b2;
					(_a3 = context.spawned.stdout) == null
						? void 0
						: _a3.off("data", wait);
					(_b2 = context.spawned.stderr) == null
						? void 0
						: _b2.off("data", wait);
				...
				stdOut &&
					((_a2 = context.spawned.stdout) == null
						? void 0
						: _a2.on("data", wait));
				stdErr &&
					((_b = context.spawned.stderr) == null
						? void 0
						: _b.on("data", wait));
				...
				if (param === "--") {
					append2(
						data
							...
							.flatMap(
								(item) =>
									(isPathSpec(item) && toPaths(item)) || item
							)
					);
					break;
				}
...
var GHSyncPlugin = class extends import_obsidian.Plugin {
	async SyncNotes() {
		...
		new import_obsidian.Notice(
			"GitHub Sync: Successfully set remote origin url"
		);
		try {
			await git.pull(
				"origin",
	...
	async onload() {
		...
		if (!isNaN(this.settings.syncinterval)) {
			...
			if (interval >= 1) {
				try {
					(0, import_set_interval_async.setIntervalAsync)(
						async () => {
		...
		try {
			...
			if (statusUponOpening.behind > 0) {
				if (this.settings.isSyncOnLoad == true) {
					...
					new import_obsidian.Notice(
						"GitHub Sync: " +
							statusUponOpening.behind +
							" commits behind remote.\nClick the GitHub ribbon icon to sync."
					);
				}
			} else {
				new import_obsidian.Notice(
					"GitHub Sync: up to date with remote."
				);
			}
		} catch (e) {}
	}
...
var GHSyncSettingTab = class extends import_obsidian.PluginSettingTab {
	...
	display() {
		...
		new import_obsidian.Setting(containerEl)
			...
			.setDesc(
				"If git is not findable via your system PATH, then provide its location here"
			)
			.addText((text) =>
				text
					.setPlaceholder("")
		...
		new import_obsidian.Setting(containerEl)
			...
			.setDesc(
				"Automatically sync when you start obsidian if there are unsynced changes"
			)
			.addToggle((toggle) =>
				toggle
		...
		new import_obsidian.Setting(containerEl)
			...
			.setDesc(
				"Set a positive integer minute interval after which your vault is synced automatically. Auto sync is disabled if this field is left empty or not a positive integer. Restart Obsidan to take effect."
			)
			.addText((text) =>
				text
